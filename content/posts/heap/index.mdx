...
class Heap<T = any> {
  ...
  removeWithMinimumPriority(): T | null {
    ...
    while (leftChild && rightChild) {
      ...
      let smallerNode = currentNode;

      // currentNode's priority is strictly greater than leftChild's priority
      if (this.comparator(currentNode, leftChild) < 0) {
        smallerIndex = Heap.getLeftChildIndex(currentIndex);
        smallerNode = leftChild;
        // currentNode's priority is strictly greater than rightChild's priority
      } else if (this.comparator(currentNode, rightChild) < 0) {
        smallerIndex = Heap.getRightChildIndex(currentIndex);
        smallerNode = rightChild;
      }

      if (smallerIndex === currentIndex) break;
      // Swap currentIndex with smallerOne
      this.heap[smallerIndex] = currentNode;
      this.heap[currentIndex] = smallerNode;

      currentIndex = smallerIndex;
      currentNode = this.heap[smallerIndex];
...
console.log(heap.removeWithMinimumPriority());
console.log(heap.removeWithMinimumPriority());
console.log(heap.removeWithMinimumPriority());
console.log(heap.removeWithMinimumPriority());
console.log(heap.removeWithMinimumPriority());
